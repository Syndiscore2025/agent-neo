...
export class ChatPanel implements vscode.WebviewViewProvider {
    ...
    private auggieModelIds: Set<string> = new Set();
    ...
    private auggieSessionId: string | null = null;
     ...
     */
    private async handleAutoRun(task: string, model?: string, fromQueue = false) {
        if (!task) { return; }
        ...
        if (!fromQueue) { this.post({ type: 'userMessage', message: task }); }
    ...
    public stopAuggie() {
        ...
        if (this.auggie.isRunning()) {
            ...
            this.post({ type: 'streamRunDone' });
            if (hadQueue) { this.post({ type: 'auggieQueued', count: 0 }); }
            vscode.window.showInformationMessage(
                hadQueue ? `Auggie session stopped; ${hadQueue} queued prompt(s) cleared.` : 'Auggie session stopped.'
            );
        } else if (hadQueue) {
    ...
    public async sendToTerminalAgent(initial?: string) {
        ...
        if (!isOrchestratorEnabled(settings)) {
            vscode.window.showWarningMessage(
                'Terminal Agent is off. Turn it on from the Terminal Agent settings tab first.',
            );
            return;
        }
     ...
     */
    private async handleEnhancePrompt(text: string, model?: string) {
        const raw = (text || '').trim();
        if (!raw) {
        ...
        const meta =
            'You are a prompt engineer. Rewrite the user instruction below into a ' +
        ...
        try {
            ...
            if (backend === 'auggie') {
                ...
                const resp = await this.apiClient.sendChatMessage(
                    meta, undefined, undefined, undefined, model || this.selectedModel
                );
     ...
     */
    private async pushModelCatalog() {
        const backend = vscode.workspace
            .getConfiguration('agentNeo')
            .get<string>('agentBackend', 'neo');
        if (backend === 'auggie') {
     ...
     */
    private async sendAuggieModelList() {
        try {
