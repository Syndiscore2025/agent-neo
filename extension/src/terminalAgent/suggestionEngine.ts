...
import { SessionSuggestion, SafetyFlag } from './session';
...
export function buildSuggestions(input: SuggestionInput): SessionSuggestion[] {
    const { parsed, verification, changedFiles, safetyFlags } = input;
    const out: SessionSuggestion[] = [];
    const add = (id: string, title: string, prompt?: string) => out.push({ id, title, prompt });
    ...
    if (parsed.errors.length) {
        add('fix-errors', `Investigate ${parsed.errors.length} error(s) reported by the agent`,
            `The previous run reported ${parsed.errors.length} error(s). Diagnose the root cause and fix them; re-run the relevant checks to confirm.`);
    }
    if (parsed.tests.failed > 0) {
        add('fix-tests', `Fix ${parsed.tests.failed} failing test(s)`,
            `${parsed.tests.failed} test(s) are failing after the last run. Make them pass without weakening assertions, then re-run the suite.`);
    }
    if (verification.unverifiedClaims.length) {
        add('unverified', `Verify ${verification.unverifiedClaims.length} claimed change(s) that did not alter any file`,
            `The agent claimed to change ${verification.unverifiedClaims.join(', ')}, but those files are unchanged on disk. Re-check whether the edits were actually applied.`);
    }
    if (verification.unclaimedChanges.length) {
        add('unclaimed', `Review ${verification.unclaimedChanges.length} unexpected change(s) the agent did not mention`,
            `These files changed without being mentioned: ${verification.unclaimedChanges.join(', ')}. Confirm each change is intended.`);
    }
    if (!parsed.tests.detected && changedFiles.length) {
        add('add-tests', 'Add tests for the changed files (none were detected this run)',
            `Add focused tests covering the behaviour changed in: ${changedFiles.slice(0, 10).join(', ')}.`);
    }
    ...
    if (input.status === 'completed' && parsed.tests.failed === 0 && !parsed.errors.length) {
        add('run-suite', 'Run the full test suite to confirm nothing regressed',
            'Run the project\'s full test suite and report any failures.');
    }
    if (!out.length) {
