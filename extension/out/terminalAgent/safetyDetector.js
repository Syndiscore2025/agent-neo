...
exports.detectDangerousCommands = detectDangerousCommands;
exports.redactSecrets = redactSecrets;
exports.redactSafetyFlags = redactSafetyFlags;
const DANGER_RULES = [
...
const REDACTORS = [
    ...
    s => s.replace(/([A-Za-z0-9_]*(?:KEY|TOKEN|SECRET|PASSWORD|PASSWD|PWD)[A-Za-z0-9_]*)(\s*[:=]\s*)(["']?)([^\s"']+)(["']?)/gi, (_m, name, sep, q1) => `${name}${sep}${q1}${REDACTED}${q1}`),
];
...
function redactSafetyFlags(flags) {
    return flags.map(f => ({ ...f, detail: redactSecrets(f.detail) }));
}
//# sourceMappingURL=safetyDetector.js.map